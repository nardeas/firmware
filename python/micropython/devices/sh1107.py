"""
* SSH1107 display controller: reference implementation for oled 1.12" module
"""
from micropython import const
import framebuf
import time

# 1. Set Column Address 4 lower bits (POR = 00H) 
_LOW_COLUMN_ADDRESS = const(0x00) 
# 2. Set Column Address 4 higher bits (POR = 10H)  
_HIGH_COLUMN_ADDRESS = const(0x10) 
# 3. Set Memory addressing mode
# 0x20 horizontal addressing; 0x21 vertical addressing
_MEM_ADDRESSING_MODE = const(0x20) 
# 4. Set Contrast Control (double byte command)
_SET_CONTRAST = const(0x8100) 
# 5. Set Segment Re-map: (A0H - A1H)
_SET_SEGMENT_REMAP  = const(0xa0) 
# 6. Set Multiplex Ratio: (Double Bytes Command)
...
# 10. Set DC-DC Setting (set charge pump enable)
...
# 12. Set Page Address (using 4 low bits)
_SET_PAGE_ADDRESS = const(0xB0) 
# 13. Set Common Output Scan Direction: (C0H - C8H)
_SET_SCAN_DIRECTION = const(0xC0) 
# 14. Set the frequency of the internal display clocks (DCLKs).
...
# 17. Set Display Start Line (double byte command)
...
class SH1107_I2C(SH1107):
    def __init__(self, width, height, i2c, res=None, address=0x3c,
...
if __name__ == '__main__':
    ...
    i2c = I2C(1, scl=7, sda=6)
    for dev in i2c.scan():
        print(hex(dev))

    disp = SH1107_I2C(128, 128, i2c, rotate=180)
    disp.init()
    disp.wake()
